...
class BoardGameController: UIViewController {
    ...
    // кол-во пар уникальных карточке
    var cardsPairsCounts = 32
    // сущность "Игра"
    ...
    private func getStartButtonView() -> UIButton {
        ...
        // подключаем обработчки нажатия на кнопку
        // до IOS 14
...
//        }), for: .touchUpInside)
    ...
    private func getBoardGameView() -> UIView {
        // отступ игрового поля от ближайщих элементов
        ...
        // y
        // Получаем доступ к текущей сцене окна
           if let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
              let window = windowScene.windows.first {
               // Определяем отступ сверху от границ окна до Safe Area
               let topPadding = window.safeAreaInsets.top
    ...
    private func getCardsBy(modelData: [Card]) -> [UIView] {
        // хранилище для предоставления карточек
        var cardViews = [UIView]()
        // фабрика карточек
        let cardViewFactory = CardViewFactory()
        // перебираем массив карточке в Модели
        for (index, modelCard) in modelData.enumerated() {
            ...
            // добовляем второй экземпляр карты
